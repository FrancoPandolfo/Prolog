%split recibe un string y lo transforma en una lista de strings
%dividiendo el string original a partir de los espacios
%con ayuda de la funcion de la biblioteca de prolog split_string
%cuyo segundo parametro es el patron de division
split(String,L) :-
...
oracion(X) :- sujeto(X, U), predicado(U, V).

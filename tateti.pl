...
ingreso_invalido :- write('\n'),write('a ingresado una posicion invalida, porfavor ingrese nuevamente'),nl,movimiento_humano.

comprobar_validez(L) :- lugar_valido(L),assert(o(L)).
comprobar_validez(L) :- \+lugar_valido(L),ingreso_invalido.

movimiento_humano:- write('Ubique la ficha virtual -Casilleros [1-9]-'), read(L),comprobar_validez(L).
...
%Imprime el tablero
imp_tablero:- separador,nl,imp_l(1),barra,imp_l(2),barra,imp_l(3),barra,nl,imp_l(4),barra,imp_l(5),barra,imp_l(6),barra,nl,imp_l(7),barra,imp_l(8),barra,imp_l(9),barra,nl.
